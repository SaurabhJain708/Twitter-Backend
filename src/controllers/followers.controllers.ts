...
export const subscribe = AsyncHandler(async(req:AuthRequest,res:Response)=>{
    ...
    const checksubexist = await Subscriber.findOne({subscriber_id:checksubscriber._id,subscribedTo_id:checksubscribedTo._id})
    if(checksubexist){
        throw new ApiError(400,"user is already subscribed to this account")
    }
    const createSubscription = await Subscriber.create({subscriber_id:checksubscriber._id,subscribedTo_id:checksubscribedTo._id})
    if(!createSubscription){
        throw new ApiError(500,"Something went wrong while subscribing")
    }
    await User.findByIdAndUpdate(checksubscribedTo._id,{$inc:{subscribers:1}})
...
export const unSubscribe = AsyncHandler(async(req:AuthRequest,res:Response)=>{
    ...
    const deleteSubscription = await Subscriber.findByIdAndDelete(checksubexist._id)
